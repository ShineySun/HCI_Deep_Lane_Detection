#python3 -m bdd_data.show_labels --image-dir bdd100k/images/100k/val \-l bdd100k/labels/bdd100k_labels_images_val.json \-o bdd100k/val
import json
import argparse
import urllib
    ...
    def show_image(self):
        ...
        if self.with_image:
            if 'url' in frame and len(frame['url']) > 0:
                ...
                #im = np.array(img, dtype=np.uint8)
                print("%%%%%%%%%%여기다2%%%%%%%%")
            self.ax.imshow(im, interpolation='nearest', aspect='auto', cmap='gray')
            # plt.pause(100)
        else:
            self.ax.set_xlim(0, self.image_width - 1)
            self.ax.set_ylim(0, self.image_height - 1)
